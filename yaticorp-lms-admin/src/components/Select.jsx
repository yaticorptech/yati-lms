/**
 * The admin panel's dropdown — a drop-in replacement for a native <select>.
 *
 * Why not the native one: its open list is drawn by the operating system, so
 * the page cannot size or style it. A long list (forty-odd job categories)
 * opened as a tall grey sheet over the whole window, in the OS font, nothing
 * like the rest of the panel. This one is drawn by the page:
 *
 *   - under the field, flipping above it when there is no room below, never
 *     taller than the screen, scrolling inside itself;
 *   - with a search box once there are more than eight choices;
 *   - on a phone, as a popup in the middle of the screen — where a thumb
 *     reaches easily — headed with the field's name, with large rows;
 *   - keyboard-complete: arrows, Home/End, Enter, Escape, and typing a letter
 *     jumps to the next choice that starts with it; announced as a listbox.
 *
 * Same interface as <select>, so swapping is one word: `value`, `onChange`
 * (called with `{ target: { value, name, id } }`, which is all any caller
 * here reads), `className` for the closed field, `disabled`, `id`,
 * `aria-label`, and <option> children — mapped, conditional or in fragments.
 *
 * `multiple`: `value` is an array and so is the one handed to onChange. The
 * closed field shows what is picked as small tags; the list gets a checkbox
 * per row, stays open while choices are ticked, and has Clear and Done.
 * For a long set of tags this replaces a wall of toggle buttons.
 */
