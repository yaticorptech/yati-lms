...
export default function ResumeSection() {
    ...
    // After a quick upload, the server may still be reading skills. Look for
    // the result every few seconds for about a minute, then stop quietly.
    const pollRef = useRef(null);
    const pollForSkills = (uploadedAt) => {
        clearInterval(pollRef.current);
        let tries = 0;
        pollRef.current = setInterval(async () => {
            tries += 1;
            try {
                const r = await api.get('/user/resume');
                const row = r.data.resume;
                const same = !uploadedAt || !row?.uploadedAt || new Date(row.uploadedAt).getTime() === new Date(uploadedAt).getTime();
                if (row && same && row.parseStatus !== 'parsing') {
                    clearInterval(pollRef.current);
                    setResume(row);
                    setAts(r.data.ats);
                    setNotice(row.parseStatus === 'parsed'
                        ? `${row.skills?.length || 0} skills read from your resume and added to your ATS resume.`
                        : 'Saved. The skill reader was unavailable, so your ATS resume uses your courses for now.');
                }
    ...
    const pick = async (file) => {
        ...
        try {
            ...
            setAts(r.data.ats);
            if (r.data.parsed) {
                setNotice(`Uploaded — ${r.data.resume.skills?.length || 0} skills read from it and added to your ATS resume.`);
            } else if (r.data.parsing) {
                // The file is saved; the skill reader is still working on it
                // in the background. Check back a few times for the result.
                setNotice('Uploaded and saved. Reading the skills from it…');
                pollForSkills(r.data.resume?.uploadedAt);
            } else {
                setNotice('Uploaded and saved. The skill reader was unavailable, so your ATS resume uses your courses for now.');
            }
        } catch (err) {
    ...
    const download = async () => {
        ...
        try {
            ...
            saveToDrive(pdf, {
                name: 'ATS_Resume.pdf',
                description: 'Your resume, generated by YATICORP.',
                reason: 'So a copy of your resume is kept in your own Google Drive.'
            });
        } catch {
        ...
        <section aria-labelledby="your-resume-title" className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm divide-y divide-slate-100">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-3 sm:px-5">
                <div className="flex items-center gap-3">
                    <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-indigo-500 to-violet-600 text-white shadow-md shadow-indigo-200"><FileText size={20} /></span>
                    <div>
                        <h2 id="your-resume-title" className="text-base font-black tracking-tight text-slate-900 sm:text-lg">Your Resume <span className="text-amber-400">✨</span></h2>
            ...
            {/* ── Status ───────────────────────────────────────────────── */}
            <div className="relative overflow-hidden border-l-4 border-l-emerald-500 px-4 py-3 sm:px-5">
                ...
                {loading ? (
                    ...
                    <div className="relative flex flex-col gap-4 sm:flex-row sm:items-center">
                        <div className="relative shrink-0 self-center sm:self-auto">
                            <span className="flex h-20 w-20 items-center justify-center rounded-full border-2 border-dashed border-emerald-200 bg-white">
                                <span className={`flex h-12 w-12 items-center justify-center rounded-xl ${resume ? 'bg-gradient-to-br from-emerald-400 to-teal-500 text-white' : 'bg-slate-100 text-slate-400'}`}><FileText size={22} /></span>
                            </span>
                        ...
                        <div className="min-w-0 flex-1">
                            {resume ? (
                                <>
                                    ...
                                    <h3 className="mt-1 truncate text-base font-bold text-slate-900 sm:text-lg" title={resume.filename}>{resume.filename || 'Your resume'}</h3>
                                ...
                                <>
                                    ...
                                    <h3 className="mt-1 text-base font-bold text-slate-900 sm:text-lg">No resume uploaded yet</h3>
            ...
            {/* ── Action tiles ─────────────────────────────────────────── */}
            {/* Remove only exists once there is something to remove; a disabled
                "Remove" beside "Upload" used to greet every new student. */}
            <div className={`grid gap-2.5 px-4 py-3 sm:px-5 ${resume ? 'md:grid-cols-[1fr_1fr_1.5fr]' : 'md:grid-cols-[1fr_1.5fr]'}`}>
