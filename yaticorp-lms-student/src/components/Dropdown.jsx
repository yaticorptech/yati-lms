/**
 * The app's own dropdown, shared by the interview and jobs sections.
 *
...
export default function Dropdown({
    ...
    label, icon: Icon, className = '', panelClassName = '', accent = 'violet',
    placement = 'sheet'
}) {
    // 'sheet'  — a bottom sheet on a phone, a panel under the field from sm up.
    // 'panel'  — a panel under the field at every width, which is how the
    //            Target role box beside these already behaves. In a form where
    //            the two sit together, one field answering with a sheet and its
    //            neighbour with a panel reads as two different controls.
    const asPanel = placement === 'panel';
    ...
    // A panel is placed against its field, which says nothing about whether it
    // fits on the screen. A field pushed to one side by a wrap — the
    // leaderboard's period picker on a narrow card, sitting at the left with a
    // right-aligned 160px panel — put the panel's left edge at -11px. So once
    // it is open, measure it and slide it back inside.
    useEffect(() => {
        ...
        // Written to the node rather than held in state: this is a measurement
        // of what the browser did, and feeding it back through a render only
        // to measure again is the cascade the effect rules warn about.
        if (over > 0) el.style.transform = `translateX(${-Math.round(over)}px)`;
        else if (under > 0) el.style.transform = `translateX(${Math.round(under)}px)`;
    }, [open, asPanel]);
    ...
    return (
        <div ref={rootRef} className="relative">
            ...
            {open && (
                <>
                    {/* A sheet covers the page, so it needs something behind it
                        both to dim what it covers and to catch the tap that
                        dismisses it. A panel sits under its own field and the
                        outside click is already handled above. */}
                    {!asPanel && <div className="fixed inset-0 z-[150] bg-slate-900/40 sm:hidden" onClick={close} aria-hidden="true" />}
                    <div ref={panelRef}
                        className={`${asPanel
                        ? 'absolute left-0 right-0 top-full z-30 mt-1 flex max-h-72 flex-col rounded-xl border border-slate-200 bg-white shadow-lg'
                        : `fixed inset-x-0 bottom-0 z-[151] flex max-h-[70vh] flex-col rounded-t-3xl border border-slate-200 bg-white shadow-2xl
                        ...
                        {/* A phone has nothing to tap beside the sheet — it
                            covers the field that opened it — so the way out has
                            to be on the sheet itself. A panel leaves the page
                            in view, so the bar would only be clutter. */}
                        {!asPanel && (
                    ...
                    <ul
                        ...
                        className={asPanel
                            ...
                            : 'min-h-0 flex-1 overflow-y-auto p-2 pb-[calc(0.5rem+env(safe-area-inset-bottom))] sm:pb-2'}
                    >
                        {options.map((o, i) => {
                            ...
                            return (
                                <li key={o.value} role="option" aria-selected={on}
                                    ...
                                    className={`flex cursor-pointer items-center gap-2.5 text-sm transition-colors ${
                                        asPanel ? 'px-4 py-2.5' : 'rounded-xl px-3.5 py-3 font-semibold sm:py-2.5'
                                    } ${on ? `${tone.on} font-medium` : cursor === i ? 'bg-slate-100 text-slate-800' : 'text-slate-700'}`}>
