...
Before a student sees the board they complete, in order, whatever Platform
Settings requires: **Identity (Aadhaar) → LinkedIn → Resume → Profile → Jobs**.
The server owns the state; the UI only renders the server's latest answer.
An administrator can switch any of the four off (Settings → Jobs → Job Access
Verification requirements); a switched-off step disappears from the flow.

| | |
|---|---|
| API | `src/jobboard/routes/verification.js`, mounted at `/api/jobs/verification` |
| State | `src/jobboard/services/jobVerificationService.js` (the machine) + `models/JobVerification.js` (one row per user, `jobboard_job_verifications`) |
| Requirements | `src/jobboard/services/jobVerificationRequirements.js`, reading `Setting.jobVerification` (cached 30 s, cleared on admin save) |
| Gate | `src/jobboard/middleware/requireJobAccess.js` — 403 `{ code: 'JOB_VERIFICATION_REQUIRED' }` on the listings and saved-jobs routes |
| Aadhaar providers | `src/jobboard/services/aadhaar/` — `offline-qr` (default), `mock` (dev only, refused in production), `production` (vendor stubs) |
| UI | `yaticorp-lms-student/src/jobs/verification/` — `JobVerificationFlow.jsx` is rendered by `pages/Jobs.jsx` until `canAccessJobs` is true |

**States.** `NOT_STARTED → AADHAAR_SCAN_PENDING → AADHAAR_QR_VALIDATED →
OTP_SENT → AADHAAR_VERIFIED → LINKEDIN_PENDING → LINKEDIN_ADDED →
RESUME_PENDING → RESUME_ADDED → PROFILE_PENDING → JOB_ACCESS_GRANTED`. Every
response is the same `view` shape: `state`, `canAccessJobs`, `nextStep`
(`identity` | `linkedin` | `resume` | `profile` | `jobs`), `requirements`,
`options` (availability and job-type vocab) and per-step `steps.*` summaries.
A step out of order answers 409 `WRONG_STEP` with the step still owed. A
finished step can be revisited from the progress bar but not redone unless
the student asks ("Scan a different card", "Use a different profile",
"Upload a different resume").

**Identity, offline-qr.** The student scans the Secure QR on the Aadhaar card
(camera, native `BarcodeDetector` when available, or a photo upload). The
server decodes it (`secureQr.js`: big decimal → bytes → gunzip → `0xFF` fields),
keeps only the name, the last four digits and the hashed mobile, and answers
with a card summary. The student then types the mobile number linked with the
Aadhaar; the server checks it against the hash inside the QR (SHA-256 iterated
by the Aadhaar's last digit) and sends a six-digit OTP through
`services/smsService.js`. With no SMS provider configured the OTP is written to
the server log and the UI shows a "Test mode" note. Limits: five attempts,
five resends, 30-second resend cooldown, 30-minute scan session.

**Identity, mock.** `AADHAAR_PROVIDER=mock` (never in production). Any QR
text of the form `MOCK-AADHAAR:<name>:<10-digit mobile>` validates, the OTP
is `AADHAAR_MOCK_OTP` (default `123456`), nothing is sent anywhere, and the
UI labels every screen "Development mode".

**Identity, production.** `ProductionAadhaarVerificationProvider.js` is the
hook for an authorised UIDAI AUA/KUA or a licensed vendor: fill in the
`TODO(vendor)` methods (`startVerification`, `validateQRCode`, `sendOTP`,
`verifyOTP`, `getVerificationStatus`) against `AADHAAR_PROVIDER_BASE_URL` /
`AADHAAR_PROVIDER_API_KEY`. Until then it answers `PROVIDER_UNAVAILABLE`.

**LinkedIn.** Format-checked only (`linkedinProfileService.js`), normalised to
`https://www.linkedin.com/in/<handle>`. The UI says "added", never "verified":
a URL proves no ownership. The service is the seam for OAuth/OIDC later.

**Resume.** Off by default (Settings → Jobs → Require resume turns it on). Reuses the LMS's one resume per student: the file goes through
`POST /api/user/resume` (PDF, DOC, DOCX or an image, 5 MB, stored privately on
Bunny), and `POST /verification/resume/confirm` ticks the step once the
`ResumeProfile` record exists. A resume uploaded earlier on the profile page
counts; deleting it there makes the step owed again.

**Profile.** `POST /verification/profile` with `location` (+ optional
`coords` `[lon, lat]`), `skills`, `availability` (`part-time` | `full-time` |
`weekends`) and `jobTypes` (`it` | `non-it` | `local` | `part-time` |
`internship` | `remote`). Location and skills are required only while the
admin requires them. The saved profile pre-fills the Jobs search form.

**What is never stored.** The OTP (hashed into an HMAC transaction id that is
itself encrypted), the full Aadhaar number, the raw QR payload. The provider
reference and OTP transaction are sealed with AES-256-GCM (`utils/secretBox.js`,
key from `VERIFICATION_ENCRYPTION_KEY` or derived from `JWT_SECRET`). The audit
collection (`jobboard_job_verification_audit`) records events, never values.

**Env.** `AADHAAR_PROVIDER`, `AADHAAR_MOCK_OTP`, `AADHAAR_UIDAI_CERT_PATH`,
`AADHAAR_PROVIDER_BASE_URL` / `_API_KEY`, `VERIFICATION_ENCRYPTION_KEY`, and the
SMS keys (`FAST2SMS_API_KEY`, `MSG91_*`, `TWILIO_*`). See the server README.
